...
const ProfileSecurityScreen = () => {
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(scanningLineAnim, {
          toValue: 1,
          duration: 1000, // Move down in 1 second
          useNativeDriver: true,
        }),
        Animated.timing(scanningLineAnim, {
          toValue: 0,
          duration: 1000, // Move up in 1 second
          useNativeDriver: true,
        }),
      ])
    ).start();
  ...
  useEffect(() => {
    ...
    if (modalVisible) {
      ...
      timer = setTimeout(() => {
        stopScanningAnimation(); // Stop animation before showing native prompt
        handleBiometricAuthentication();
      }, 5000); // 5 seconds simulation
    } else {
      stopScanningAnimation();
      clearTimeout(timer); // Clear timeout if modal is closed prematurely
    }
  ...
  const translateY = scanningLineAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [-30, 30], // Adjust these values based on the fingerprint icon size
  });
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalView}>
            ...
            <Text style={styles.modalText}>
              Place your finger on the sensor for 5 seconds to enable biometric security.
            </Text>
